-- [[ TMG MAINFRAME: HELI OPTICS STATE MATRIX ]]
-- Unified registry for thermal optics, searchlight sync, and tracking telemetry
HeliState = {
    -- Constants & Keybinds
    fov_max = 80.0,
    fov_min = 10.0,
    ...
    -- State Nodes
    helicam = false,
    fov = 45.0, -- Start zoom level
    vision_state = 0, -- 0: Normal, 1: Night, 2: Thermal
...
local function ChangeVision()
    -- Cyclical vision toggle: Normal -> Night -> Thermal -> Normal
...
-- [[ TMG OPTICS ENGINE: KINETIC CAMERA LOGIC ]]
...
local function CheckInputRotation(cam, zoomvalue)
    ...
    if rightX ~= 0.0 or rightY ~= 0.0 then
        ...
        -- Vertical Clamp: 20.0 (High) to -89.5 (Low)
...
local function HandleZoom(cam)
    ...
    -- Zoom Interpolation: 5% smoothing factor
...
local function GetVehicleInView(cam)
    ...
    -- Angle to Vector conversion for Raycasting
...
local function RenderVehicleInfo(vehicle)
    ...
    local speed = math.ceil(GetEntitySpeed(vehicle) * 3.6) -- KPH Conversion
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn and PlayerJob.type == 'leo' and PlayerJob.onduty then
            if IsPlayerInPolmav() then
                ...
                -- 1. UTILITY NODES (Spotlight & Rappel)
                if IsHeliHighEnough(heli) then
                    if IsControlJustPressed(0, 51) then -- Toggle Helicam (E)
                        PlaySoundFrontend(-1, 'SELECT', 'HUD_FRONTEND_DEFAULT_SOUNDSET', false)
                        HeliState.helicam = true
                        SendNUIMessage({ type = 'heliopen' })
                    end
                    if IsControlJustPressed(0, 154) and (GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed) then -- Rappel (X)
                        PlaySoundFrontend(-1, 'SELECT', 'HUD_FRONTEND_DEFAULT_SOUNDSET', false)
                        TaskRappelFromHeli(lPed, 1)
                    end
                end

                if IsControlJustPressed(0, 74) and (GetPedInVehicleSeat(heli, -1) == lPed or GetPedInVehicleSeat(heli, 0) == lPed) then -- Spotlight (H)
                ...
                if HeliState.helicam then
                    ...
                    PushScaleformMovieFunctionParameterInt(0) -- 0: None, 1: LSPD
                    ...
                    while HeliState.helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
                        if IsControlJustPressed(0, 51) then -- Close (E)
                            HeliState.helicam = false
                            print("^5[TMG]^7 Optical node disengaged.")
                        elseif IsControlJustPressed(0, 25) then -- Vision (Right Click)
                            PlaySoundFrontend(-1, 'SELECT', 'HUD_FRONTEND_DEFAULT_SOUNDSET', false)
                            ChangeVision()
                        end
                        ...
                        if HeliState.locked_on_vehicle then
                            if DoesEntityExist(HeliState.locked_on_vehicle) then
                                ...
                                if IsControlJustPressed(0, 22) then -- Unlock (Space)
                                    PlaySoundFrontend(-1, 'SELECT', 'HUD_FRONTEND_DEFAULT_SOUNDSET', false)
                                    local rot = GetCamRot(cam, 2)
...
-- [[ TMG SCAN ENGINE: ASYNC TELEMETRY ]]
